...
public class ZombieAI1 : MonoBehaviour
{
    ...
    public float searchRadius = 10f; // Detection range
    ...
    void Start()
    {
        ...
        Vector3 randomPoint = GetReachableRandomNavMeshPosition(transform.position, searchRadius, agent);
    ...
    Vector3 GetReachableRandomNavMeshPosition(Vector3 origin, float radius, NavMeshAgent navAgent)
    {
        ...
        for (int i = 0; i < attempts; i++)
        {
            ...
            if (NavMesh.SamplePosition(randomDirection, out NavMeshHit hit, radius, navAgent.areaMask))
            {
                if (IsPathValid(navAgent, hit.position))
                {
                    return hit.position;
                }
            }
    ...
    bool IsPathValid(NavMeshAgent navAgent, Vector3 targetPosition)
    {
...
void OnDrawGizmos()
{
    ...
    void ChasePlayer()
    {
        ...
        if (Vector3.Distance(transform.position, player.position) > searchRadius * 1.5f)
        {
            isChasing = false;
            agent.speed = speed;
            GoToNextPatrolPoint();
